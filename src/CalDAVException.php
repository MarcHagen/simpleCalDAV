...
class CalDAVException extends Exception
{
    ...
    public function __toString()
    {
        ...
        if (!empty($this->requestBody)) {
            if (!preg_match('#^Content-type:.*?(text/calendar|application/xml).*?$#', $this->requestHeader, $matches)) {
                $dom->loadXML($this->requestBody);
                $string .= htmlentities($dom->saveXml());
            } else {
                $string .= htmlentities($this->requestBody);
            }
        }
        else {
        ...
        $string .= '<br /><br />last response:<br /><br />';

        $string .= $this->responseHeader;
        if (!empty($this->responseBody)) {
            if (!preg_match('#^Content-type:.*?(text/calendar|application/xml).*?$#', $this->responseHeader, $matches)) {
        ...
        $string .= '<br /><br />Trace:<br />' . $this->getTraceAsString();
        $string .= '</pre>';
